...
from lookups import DestinationDatabase, InputTypes, FredEconomicDataWebScrape,ErrorHandling
...
def predict_values_using_arima(df, dependent_variable, independent_variables, arima_order, future_periods):
    ...
    try:
        ...
        error_string_prefix = ErrorHandling.ARIMA_ERROR.value
...
def process_prediction_results(predicted_values, confidence_intervals,dependent_variable, latest_date):
    ...
    try:
        ...
        error_string_prefix = ErrorHandling.PROCESS_PREDICTION_RESULTS_ERROR.value
        error_string_suffix = str(e)
        show_error_message(error_string_prefix,error_string_suffix)
    finally:
...
def get_forecast_gdp(db_session, schema_name=DestinationDatabase.SCHEMA_NAME):
    ...
    try:
        ...
        error_string_prefix = ErrorHandling.GDP_FORECAST_ERROR.value
        error_string_suffix = str(e)
        show_error_message(error_string_prefix,error_string_suffix)
    finally:
